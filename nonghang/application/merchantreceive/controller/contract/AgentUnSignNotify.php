...
class AgentUnSignNotify extends Controller
{
    public function unsign()
    {
        ...
        {
            $responseStr = '验签失败！';
            Log::info("AgentUnSignNotify验签结果：".$responseStr);
        }
        else
        {
...
//            $responseJson = json_encode($responseBody);
        ...
        Log::info("AgentUnSignNotify向后台发送的报文加密前为：".$responseJson);
        Log::info("AgentUnSignNotify向后台发送的报文加密后为：".$responseStr);
        //将加签名之后的报文发送给浏览器
